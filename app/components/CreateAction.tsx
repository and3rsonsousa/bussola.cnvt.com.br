		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				{mode === "day" ? (
					...
					<ButtonCNVT
						className="fixed right-2 bottom-3 rounded-full"
						rounded
					>
							...
							onClick={() => {
								...
								if (!action.date) {
									toast({
										...
										description:
											"Defina a data que a sua ação deve acontecer.",
									});
									return false;
								}
...
export function StateSelect({
	...
	return (
		<Select value={state} onValueChange={(value) => onValueChange(value)}>
			<SelectTrigger
				className={`button-trigger inline-flex w-auto gap-2`}
			>
				<div
			...
			<DropdownMenuContent className="glass">
				{people.map((person) => (
					<DropdownMenuCheckboxItem
						...
						onClick={(event) => {
							const checked = responsibles.includes(
								person.user_id
							);
						...
						title={
							action.date
								? format(
										...
										" por " + formatActionTime(action.time)
								  )
								: "Ação sem data"
						}
						variant="ghost"
