						...
						<div className="flex gap-2">
							...
							{[
								...
								},
								{
									...
									title: "Feed",
									description:
										"Ver as ações do feed do Instagram",
...
function DelayedActions({ actions }: { actions: Action[] }) {
	...
	useEffect(() => {
		if (query.length >= 1) {
			...
			setFiltered(() =>
				actions.filter((action) => regex.test(action.title))
			);
		} else {
			setFiltered(() => actions);
		}
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex items-center gap-2">
							...
							<Button
								size={"sm"}
								variant={
									order === "priority" ? "secondary" : "ghost"
								}
								onClick={() => {
									setOrder("priority");
								}}
...
function Partners({ actions }: { actions?: Action[] }) {
	...
	return (
		<>
			<div className="border-b"></div>
			<div className="px-2 md:px-8 py-8 lg:py-24">
				...
				{partners.length > 0 ? (
					...
					<div className="grid place-content-center p-4 text-center">
						<div className="mb-2 text-4xl font-semibold tracking-tighter text-rose-600">
							Nenhum <span className="font-bold">PARCEIRO</span>{" "}
...
const ActionsProgress = () => {
	...
	const nextMonthActions = getMonthsActions(
		actions,
