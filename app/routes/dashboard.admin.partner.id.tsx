...
export const action = async ({ request }: ActionFunctionArgs) => {
	...
	let voice = Object.keys(entries)
		.map((entry) =>
			/voice\d*/.test(entry) ? Number(entries[entry]) : null
		)
		.filter((n) => n !== null);
...
function Voice({
	...
	return (
		<div className="py-4">
			...
			{viewDescription && (
				<div className="bg-card my-4 rounded p-4">
					{voice.description}
				</div>
			)}
        ...
        {value} - {defaultValue}
			...
			<div className="mt-4 rounded py-4 text-2xl">
				{examples[value[0]]}
			</div>
		</div>
	);
